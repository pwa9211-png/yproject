	...
	2.  【强制工具调用】：对于任何问题，只要它涉及：
	    - 当前日期和时间之后的事件、新闻、天气、股价、指数、数据。
	    - 任何需要验证的、具体的人名、地名、公司名、产品名。
	    你**必须**调用 'web_search' 工具进行搜索。绝对禁止猜测或使用内部知识回答。
	...
	5.  【执行流程】：如果需要搜索，请调用工具。收到工具结果后，直接基于该结果回答并附上来源。如果问题不涉及实时事实（例如“你好”、“什么是光合作用”），可以正常回答。`;
	    ...
	    for (let attempt = 1; attempt <= 5; attempt++) {
	        ...
	        try {
	            ...
	            console.log('[AI] 模型返回的消息:', choice.message);
	            if (!choice.message) {
