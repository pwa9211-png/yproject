// lib/ai.js
// 目的：切换到智谱 GLM-4 Plus 模型，并启用其内置的联网搜索（web_search）功能。
import { OpenAI } from 'openai'; 

// 🚨 核心配置：使用智谱的 API Key 和官方 Base URL
// 强烈建议在 Vercel 或 .env.local 中设置这些变量
const GLM_API_KEY = process.env.GLM_API_KEY;
// 智谱 GLM-4 API 的官方 Base URL (通常以 /v4 结尾)
...
export async function GoogleGenAI(history, aiRole) { // 保持函数名不变以兼容 chat.js
    ...
    try {
        const completion = await aiClient.chat.completions.create({
            // 🚨 关键：模型名称设置为 GLM-4 Plus
            model: 'glm-4-plus', 
            messages: [
                { role: 'system', content: systemInstruction },
                ...finalMessages
            ],
            // ⭐️ 关键：启用 GLM-4 的联网搜索工具
