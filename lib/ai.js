...
export async function performWebSearch(query) {
  ...
  const body = {
    search_query: query,
    search_engine: 'search_pro',      // 官方高阶引擎
    search_intent: false,             // 直接搜索，不先识别意图
    count: 5,
    search_recency_filter: 'oneDay',  // 只搜今天
    content_size: 'high',             // 摘要更详细
  };
  ...
  while (turns++ < 5) {
    try {
      ...
      const res = await client.chat.completions.create({
        ...
        messages    : history,
        tools,
        tool_choice : { type: 'function', function: { name: 'web_search' } }, // 强制
        temperature : 0,
      });
