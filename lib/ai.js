...
/**
 * 【诊断函数】执行模拟网络搜索
 * 我们不再调用真实的API，而是直接返回一个固定的、格式正确的字符串。
 */
async function performWebSearch(query) {
    console.log(`[工具] [诊断模式] 正在模拟搜索: "${query}"`);
    // 模拟一个真实的搜索结果，包含链接
    const fakeResult = `来源1: [中央气象台-上海天气](http://www.nmc.cn/publish/weatherarea/shanghai.htm)\n摘要: 上海今天晴，气温15~25摄氏度，微风。`;
    console.log(`[工具] [诊断模式] 返回模拟结果。`);
    return fakeResult;
}
/**
 * 运行对话与工具调用循环 (此部分保持不变)
            ...
            if (choice.finish_reason === 'tool_calls') {
                ...
                for (const toolCall of choice.message.tool_calls) {
                    if (toolCall.function.name === 'web_search') {
                        ...
                        const toolResult = await performWebSearch(query); // 这里会调用我们的模拟函数
                        history.push({
                            role: 'tool',
