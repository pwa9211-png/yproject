// lib/ai.js  2025-11-28  严格标题-链接对应+过滤用户消息版
...
export async function performWebSearch(query) {
  ...
  try {
    ...
    // ② 严格过滤：必须标题+链接，去掉用户@消息，去重链接
    const items = data.search_result
      .filter(it => it.title && it.link)                        
      .filter(it => !it.title.includes('@万能助理'))             
      .filter((it, i, arr) => arr.findIndex(x => x.link === it.link) === i)
      .slice(0, 5);

    if (items.length === 0) return `未找到今天关于“${query}”的有效新闻。`;
...
/**
 * 获取指定日期上证指数收盘值（东方财富历史 K 线）
 * date: YYYY-MM-DD
 */
export async function fetchSZZSHistory(date) {
  const url = `https://push2his.eastmoney.com/api/qt/stock/kline/get?secid=1.000001&fields1=f1,f3,f5,f6&fields2=f51,f52,f53,f54,f55,f56,f57,f58&klt=101&fqt=1&beg=${date}&end=${date}`;
  try {
    const res = await fetch(url, { headers: { Referer: 'https://quote.eastmoney.com' } });
    if (!res.ok) throw new Error(`历史行情HTTP ${res.status}`);
    const json = await res.json();
    if (!json.data?.klines?.length) return null;
    const [, close, open, high, low] = json.data.klines[0].split(',');
    return {
      date,
      close,
      open,
