	...
	async function performWebSearch(query) {
	    console.log(`[工具] 正在执行网络搜索: "${query}"`);
	    const requestBody = {
	        search_query: query,
	        search_engine: "search_std", // 使用智谱基础版搜索引擎
	        search_intent: false,
	        count: 5, // 返回5条结果就够了
	    };
	    try {
	        ...
	        if (!response.ok) {
	            ...
	            console.error(`[工具] 搜索API请求失败: ${response.status}`, errorData);
	            return `搜索API请求失败: ${response.status}, 详情: ${JSON.stringify(errorData)}`;
	        }
	        const data = await response.json();
	        console.log('[工具] 搜索API请求成功。');
	        ...
	        console.log(`[工具] 返回格式化后的搜索结果。`);
	        return formattedResults;
	    } catch (error) {
	        console.error('[工具] 搜索请求发生网络错误:', error);
	        return `搜索请求失败: ${error.message}`;
	    }
	}
	/**
	 * 运行对话与工具调用循环 (此部分保持不变，因为它已经被验证是正确的)
	 */
	            ...
	            if (choice.finish_reason === 'tool_calls') {
	                ...
	                for (const toolCall of choice.message.tool_calls) {
	                    if (toolCall.function.name === 'web_search') {
	                        ...
	                        const toolResult = await performWebSearch(query); // 这里会调用我们全新的、正确的搜索函数！
	                        history.push({
	                            role: 'tool',
