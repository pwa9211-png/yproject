好的，這是我們經過多次測試和優化後，包含**兩輪工具呼叫邏輯**、**強化系統指令**以及**優化模擬搜索結果**的 `pages/api/chat.js` 完整程式碼。

請您使用這段程式碼**完整覆蓋**您現有的 `pages/api/chat.js` 文件，並重新部署。這次的強化指令應該能解決 AI 在第二輪交互中行為迷失或重複使用者提問的問題。

### `pages/api/chat.js` 完整程式碼
